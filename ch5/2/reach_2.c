/*
    Modeling reach as a scenario with 2 ropes instead of 1

    Rope 2, defined as 'state' will be where Bob does all his falling,
    and it will have zero cost. However, moving from the original rope
    to the state rope will cost a move

    Moves with a zero cost will be processed in the current round

    Variant of BFS known as 0-1 BFS
...
/*
    We're creating a separate function for each type of move
    that's possible on this board
*/

void add_position_up(int from_height, int to_height, int max_height,
                     positions pos, int *num_pos, int itching[],
                     board min_moves) {
...
void add_position_down(int from_height, int to_height, positions pos,
                       int *num_pos, board min_moves) {
...
void add_position_01(int from_height, positions pos, int *num_pos,
                     board min_moves) {
  int distance = 1 + min_moves[from_height][0];
    ...
    for (i = 0; i < num_cur_positions; i++) {
      ...
      if (from_state == 0) {
        add_position_up(from_height, from_height + jump_distance,
                        target_height * 2 - 1, new_positions,
                        &num_new_positions, itching, min_moves);
        add_position_01(from_height, new_positions, &num_new_positions,
                        min_moves);
      } else {
        add_position_down(from_height, from_height - 1, cur_positions,
                          &num_cur_positions, min_moves);
        add_position_10(from_height, cur_positions, &num_cur_positions, itching,
                        min_moves);
      }
    }
