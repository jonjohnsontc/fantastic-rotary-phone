/*
  Implementing Binary Search + Dynamic Programming into QOL.
...
int can_make_quality(int quality, int r, int c, int h, int w, board q) {
  ...
  // Creating table with comparisons against quality value, which is necessary
  // for the sum table
  for (i = 0; i < r; i++)
    for (j = 0; j < c; j++)
      if (q[i][j] <= quality)
  ...
  // Creating sum table used to query the solution for a given quality
  for (i = 0; i <= c; i++)
    sum[0][i] = 0;
  for (i = 0; i <= r; i++)
    sum[i][0] = 0;
  for (i = 1; i <= r; i++)
    for (j = 1; j <= c; j++)
      sum[i][j] = zero_one[i - 1][j - 1] + sum[i - 1][j] + sum[i][j - 1] -
                  sum[i - 1][j - 1];
